...
Each session is an actor: it owns a mailbox (`asyncio.Queue`) and is drained by a single long-lived worker task. The mailbox FIFO guarantees per-session linearizability (one turn at a time, in submission order); the per-session `asyncio.Lock` around `_execute_skill_turn` is defense-in-depth so that anything bypassing the mailbox still cannot interleave reads and writes of `session.messages`. Distinct sessions never block each other -- they run on independent workers and only contend on the server-wide concurrency semaphore.

`_run_skill` is the admission point. It resolves or creates the session, runs three rejection checks in order (skill mismatch, mailbox full, semaphore saturated), then enqueues a `_WorkItem` and either returns immediately (background / Tell mode) or awaits the item's per-call `asyncio.Future` (foreground / Ask mode). The worker is lazily spun up on the first push and lives until `SessionStore.shutdown()` (or `stop_session`) cancels it; between items it blocks on `mailbox.get()`, so an idle session costs one suspended task.
...
- **`max_concurrent_runs`** (default `4`) -- a server-wide `asyncio.Semaphore` acquired by the session worker around the actual `_execute_skill_turn` call. The slot is held only for the duration of in-flight execution, not for queue admission, so mailbox queueing is unaffected. Background launches that arrive while the gate is fully held return `status="saturated"` immediately so the caller can react. Foreground launches always enqueue and end up waiting on the semaphore inside the worker -- the natural Ask-mode contract of "wait until done". The saturation check is a best-effort `Semaphore.locked()` snapshot at admission time: between the snapshot and the worker's actual acquire, a slot may free up, in which case we reject pessimistically. That is the right side to err on for backpressure.
